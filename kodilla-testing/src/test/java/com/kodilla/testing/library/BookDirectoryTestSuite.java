...
class BookDirectoryTestSuite {
    ...
    @Test
    void testListBooksInHandsOfUserWithCondition() {
        // Given
        LibraryUser zero = new LibraryUser("TheZero","x","2");
        LibraryUser one = new LibraryUser("TheOne","x","3");
        LibraryUser hero = new LibraryUser("Hero","x","4");

        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);                  // [1]
        List<Book> resultListOf0Books = new ArrayList<Book>();                           // [2]
        List<Book> resultListOf1Books = generateListOfNBooks(1);                       // [3]
        List<Book> resultListOf5Books = generateListOfNBooks(5);                       // [4]
        when(libraryDatabaseMock.listBooksInHandsOf(zero))                    // [5]
                .thenReturn(resultListOf0Books);                                             // [6]
        when(libraryDatabaseMock.listBooksInHandsOf(one))                    // [7]
                .thenReturn(resultListOf1Books);                                              // [8]
        when(libraryDatabaseMock.listBooksInHandsOf(hero))                   // [9]
                .thenReturn(resultListOf5Books);                                             // [10]

        // When
        List<Book> theListOfBooks0 = bookLibrary.listBooksInHandsOf(zero);    // [11]
        List<Book> theListOfBooks1 = bookLibrary.listBooksInHandsOf(one);   // [12]
        List<Book> theListOfBooks5 = bookLibrary.listBooksInHandsOf(hero);  // [13]
        // Then
